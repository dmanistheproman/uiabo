...
export default function InfoScreen({ premium, onNavigate }) {
  ...
  return <View style={{ flex: 1 }}><PageHeader title={premium ? 'Premium' : 'Help'} onBack={() => onNavigate('home')} /><PageBody>
    <View style={styles.card}><Text style={styles.heading}>{premium ? activePremium ? 'Your Premium account is active' : 'More ways to check images' : 'Make your next share an informed one'}</Text>
      <Text style={styles.copy}>{premium ? activePremium ? 'You receive up to 60 completed text or link safety checks each calendar month. Your allowance resets at midnight Singapore time on the first day of the month.' : 'The planned Premium tier includes image + caption, image context, OCR and AI-image checks, with up to 60 successful checks per month for SGD 20/month.' : 'Paste a short English claim into Check text. UIABO looks for a checkable statement, searches published evidence, and shows a verdict, evidence strength and citations.'}</Text>
      <Text style={styles.copy}>{premium ? 'Image checks and subscription payments are not available in this prototype yet. No payment will be taken.' : 'The verdict describes what the evidence establishes. Evidence strength describes the quality of that evidence. The concern indicator runs from 0 to 100; it is not a percentage chance of being false. A zero means no concern identified in the evidence checked. Unresolved factual checks receive a provisional 50, which is not a 50% probability of being false. Earlier saved results keep their original scores.'}</Text>
      ...
      {!premium && <><Text style={styles.heading}>Check link safety</Text><Text style={styles.copy}>Paste a public webpage URL to check Google's known phishing, malware and unwanted software lists. The full URL is sent to Google Web Risk and saved in your history. Avoid private links or links containing passwords or access tokens.</Text><Text style={styles.copy}>No known threats found does not guarantee safety. This feature does not fact-check webpage content. Text and link checks share your allowance; service failures do not use a check.</Text></>}

...
async def tavily_extract(client: httpx.AsyncClient, urls: list[str], key: str) -> dict:
    data = await request_json(client, "POST", "https://api.tavily.com/extract",
        headers={"Authorization": f"Bearer {key}"},
        json={"urls": urls, "extract_depth": "basic", "format": "text", "timeout": 15})
...
async def tavily_search(client: httpx.AsyncClient, query: str, key: str) -> list[dict]:
    data = await request_json(client, "POST", "https://api.tavily.com/search",
        ...
        json={"query": query, "search_depth": "advanced", "max_results": 8,
              ...
              "auto_parameters": False, "include_domains": list(SOURCES)})
